...
from data_envi.gripper_singlebox import GripperSingleBox
from data_envi.gripper_singlecolorbox import GripperSingleColorBox
from data_envi.gripper_multicolorbox import GripperMultiColorBox
...
class IsaacManipulationTestEnviManager():
            ...
            for batch_idx in range(self.cfg['EVAL']['TEST_ENVI_BATCH_NUM']):
                ...
                while simulation_step <= self.cfg['EVAL']['INFERENCE_MAX_STEPS']:
                    ...
                    if actions_pred == None or execution_step >= actions_pred.shape[1]:
                        ...
                        if self.cfg['POLICY']['POLICY_NAME'] == 'VIRT_Isaac':
                        ...
                        elif self.cfg['POLICY']['POLICY_NAME'] in ['ACT', 'CNNMLP', 'Diffusion']:
                            end_obs = end_obs[:, -1]    # Select the lastest qpos
    ...
    def prepare_policy_input(self, end_obs, joint_obs, past_action, all_cam_images, task_instruction, status_pred):
        ...
        if 'observations/joint_observation' in self.cfg['DATA']['INPUT_KEYS']:
            ...
            if joint_obs.shape[1] >= (past_obs_len - 1) * obs_sample_interval + 1:
                ...
                if observation_is_pad is None: observation_is_pad = torch.zeros((joint_obs.shape[0], joint_obs.shape[1]), dtype = torch.bool).to(joint_obs.device)  # Left shape: (num_env, past_obs_len)
            else:
    ...
    def get_reward(self, isaac_envi):
        if self.cfg['TASK_NAME'] == 'isaac_singlebox':
            box_idx_list = []
            for box_idx_dict in isaac_envi.obj_idxs:
            ...
            box_idx_list = []
            for task_instruction, obj_idxs in zip(isaac_envi.task_instruction, isaac_envi.obj_idxs):
                box_key = task_instruction
                box_idx_list.append(obj_idxs[box_key])
            ...
            for box_idx_dict, task_instruction in zip(isaac_envi.obj_idxs, task_instruction_list):
