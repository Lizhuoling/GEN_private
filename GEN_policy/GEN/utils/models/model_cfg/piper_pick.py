...
envi_cfg = dict(
    ...
    topcam_instrinsics_path = './configs/piper/calibration.json',
    topcam2robot_extrinsics_path = './configs/piper/camera2robot.json',
    robot_default_position = [0.08, 0.0, 0.26],
    robot_operation_range = {'x': [0.05, 1.0], 'y': [-0.5, 0.5], 'z': [-0.5, 0.5]},
    ros_topic_dict = {
        "subscriptions": [
            "/camera_top/color/image_raw",
            "/camera_top/depth/image_raw",
            "/camera_arm/color/image_raw",
            "/camera_arm/depth/image_raw",
            "/can0/joints",
            "/can0/end"
        ], 
        "publishers":[
    ...
    piper_end_reach_threshold = 0.03,
    piper_step_time_interval = 0.01,
    piper_end_max_step = [0.015, 0.015, 0.015, 0.05, 0.05, 0.05, 0.05, 1.0],
)

observation_cfg = dict(
    perception_model_name = 'PiperYoloTablewareInstSegment',
    yolo_path = '/home/cvte/twilight/code/act_project/VIRT/pretrained/yolo11n-seg.pt',
    yolo_conf = 0.25,
...
execution_cfg = dict(
    ...
    target_track_handcam_offset = [-0.01, -0.01, 0.05], # For more flexibility, we can add an offset to the hand camera position.
...
def topcam_schedule_on_objects_func(mean_obj_coor3ds, obj_cls, operation_range, cur_skill):
    ...
    Description:
        ...
        x: robot forwards, y: moving belt direction, z: upwards
    Input:
    ...
    if in_range_mean_obj_coor3ds.shape[0] > 0:
        in_range_select_obj_idx = in_range_mean_obj_coor3ds[:, 1].argmax()
