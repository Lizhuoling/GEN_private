...
envi_cfg = dict(
    ...
    belt_speed = -0.00,
    obj_init_range = ((0.10, 0.10), (0.0, 0.0)),
    num_obj_per_envi = 1,
    move_mode = 'linear',
    obj_mode = ['elephant_toy',],
    container_mode = ['basket2', 'basket3', 'container', 'container2'],
...
execution_cfg = dict(
    ...
    OpenGripper_T = 10,
    # Parameters for the ScriptApproachTargetSkill.
    translation_max_dist = 0.001,
    AlignRelativeOffset = [0.00, 0, 0.2],
    PutRelativeOffset = [0, 0, 0.2],
    TargetOrientation = [0, 0, 0, 1],   # quat: (x, y, z, w)
)
...
def get_track_target_cls_func(cur_skill):
    ...
    elif (type(cur_skill) is TargetTrackSkill and cur_skill.last_skill_info_dict['call_flag'] == 2) or (type(cur_skill) is TargetTrack_ScriptPutTargetSkill) \
            or (type(cur_skill) is OpenGripperSkill) or (type(cur_skill) is ScriptPostProcessSkill):
...
def TargetTrackSkill_checkupdate(self, obs_dict, ctrl_signal):
    ...
    if update_flag.item() and self.last_skill_info_dict['call_flag'] == 1:
        ...
        return 'TargetTrack_ScriptPutTargetSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'][0, 0].item(), 'call_flag': 1}
    else:
        return None, {}
...
def TargetTrackPickSkill_checkupdate(self, obs_dict, ctrl_signal):
    ...
    if obs_dict['handcam_tracktarget_mask'].sum() == 0:   # If the track target is lost, switch to the target track skill.
        ...
        return None, {}
    elif ctrl_signal['status_pred'].item() == 1:    # The policy judges the pick succeeds.
        return 'TargetTrackSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'][0, 0].item(), 'target_track_max_pos_noise': 0.04, 'call_flag': 2, 'target_track_handcam_dist': 0.3}
    elif ctrl_signal['status_pred'].item() == 2:    # The policy judges the pick fails, so we need to return to the target track skill.
        return 'TargetTrackSkill', {'call_flag': 1}

def TargetTrack_ScriptPutTargetSkill_checkupdate(self, obs_dict, ctrl_signal):
    if ctrl_signal['norm_gripper_ctrl'][0, 0] > 0.9:
        return 'ScriptPostProcessSkill', {'norm_gripper_ctrl': 1.0, 'target_track_handcam_dist': 0.25, 'call_flag': 3,}  # No target will be detected as call_flag is 3
    else:
        return None, {}
    
def ScriptPostProcessSkill_checkupdate(self, obs_dict, ctrl_signal):
    return None, {}