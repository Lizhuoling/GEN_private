...
envi_cfg = dict(
    ...
    belt_speed = -0.00,
    obj_init_range = ((-0.1, 0.1), (0.0, 0.0)),
    num_obj_per_envi = 1,
    move_mode = 'linear',
    obj_mode = 'own_cup',
...
execution_cfg = dict(
    ...
    # Parameters for the RotateToTargetPoseSkill.
    ref_sample_point_num = 5000,
    icp_threshold = 1,
)
...
def TargetTrackPickSkill_checkupdate(self, obs_dict, ctrl_signal):
    ...
    if obs_dict['handcam_tracktarget_mask'].sum() == 0:   # If the track target is lost, switch to the target track skill.
        ...
        return 'RotateToTargetPoseSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'][0, 0].item(), 'target_ref': '/home/cvte/twilight/data/daily_objects/assets/own_cup.obj', 'call_flag': 1}
    elif ctrl_signal['status_pred'].item() == 2:    # The policy judges the pick fails, so we need to return to the target track skill.
        return 'TargetTrackSkill', {}
    
    
def RotateToTargetPoseSkill_checkupdate(self, obs_dict, ctrl_signal):
