...
from GEN.utils.inference.point_isaac_move_manipulation import *

envi_cfg = dict(
    envi_name = 'MoveInsert',
    robot_name = 'franka_panda',
    belt_speed = -0.08,
    obj_init_range = ((0.12, 0.12), (0.3, 0.3)),
    num_obj_per_envi = 1,
    move_mode = 'linear',
    obj_mode = 'box',
    container_mode = 'insert_box',
...
execution_cfg = dict(
    ...
    # Parameters for the TargetTrackTeleSkill
    continuous_track_target_flag = False,
    # Parameters for the TargetPickSkill
    targetpick_ckpt_cfg = 'isaac_move_manipulation_basicdaily.yaml',
    targetpick_ckpt_path = '/home/cvte/twilight/code/act_project/VIRT/outputs/isaac_move_manipulation_basicdaily/policy_latest.ckpt',
    # Parameters for the TargetPutSkill
    targetput_ckpt_cfg = 'isaac_move_put_MDN_memory.yaml',
    target_put_ckpt_path = '/home/cvte/twilight/code/act_project/VIRT/outputs/isaac_move_put_MDN_memory/policy_latest.ckpt',
)
...
def get_track_target_cls_func(cur_skill):
    if (type(cur_skill) is TargetTrackSkill and cur_skill.last_skill_info_dict['call_flag'] == 1) or (type(cur_skill) is TargetTrackPickSkill):
        target_cls = 'yellow'   # object
    elif (type(cur_skill) is TargetTrackSkill and cur_skill.last_skill_info_dict['call_flag'] == 2) or (type(cur_skill) is TargetTrackPutSkill):
        target_cls = 'green'    # container
    else:
...
def topcam_schedule_on_objects_func(mean_obj_coor3ds, obj_cls, operation_range, cur_skill):
    ...
    '''
    target_cls = get_track_target_cls_func(cur_skill)
    cls_valid_flag = [ele == target_cls for ele in obj_cls]
    ...
    valid_obj_flag = objs_in_range_flag & torch.tensor(cls_valid_flag, dtype = torch.bool).to(objs_in_range_flag.device)    
    in_range_mean_obj_coor3ds = mean_obj_coor3ds[valid_obj_flag]
    if in_range_mean_obj_coor3ds.shape[0] > 0:
        ...
        select_obj_idx = valid_obj_flag.nonzero()[in_range_select_obj_idx, 0]
    else:
        select_obj_idx = None
...
def TargetTrackSkill_checkupdate(self, obs_dict, ctrl_signal):
    ...
    if update_flag.item() and self.last_skill_info_dict['call_flag'] == 1:
        return 'TargetTrackPickSkill', {'call_flag': 1}
    elif update_flag.item() and self.last_skill_info_dict['call_flag'] == 2:
        return 'TargetTrackPutSkill', {'call_flag': 1}
    else:
        return None, {}
...
def TargetTrackPickSkill_checkupdate(self, obs_dict, ctrl_signal):
    if 'handcam_tracktarget_mask' not in obs_dict.keys():   # The perception module does not work in this step.
        return None, {}
    elif self.frame_id <= 1: # The initial past action is pre-defined, which may need to incorrect status prediction, so we skip it.
        return None, {}
    elif obs_dict['handcam_tracktarget_mask'].sum() == 0:   # If the track target is lost, switch to the target track skill.
        return 'TargetTrackSkill', {'call_flag': 1}
        ...
        return 'TargetTrackSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'][0, 0].item(), 'call_flag': 2, 'target_track_handcam_dist': 0.3}
    elif ctrl_signal['status_pred'].item() == 2:    # The policy judges the pick fails, so we need to return to the target track skill.
        return 'TargetTrackSkill', {'call_flag': 1}
    
def TargetTrackPutSkill_checkupdate(self, obs_dict, ctrl_signal):
    if 'handcam_tracktarget_mask' not in obs_dict.keys():   # The perception module does not work in this step.
        return None, {}
    elif self.frame_id <= 1: # The initial past action is pre-defined, which may need to incorrect status prediction, so we skip it.
        return None, {}
    if obs_dict['handcam_tracktarget_mask'].sum() == 0:   # If the track target is lost, switch to the target track skill.
        return 'TargetTrackSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'][0, 0].item(), 'call_flag': 2}
    elif 'status_pred' not in ctrl_signal.keys():
        return None, {}
    elif ctrl_signal['status_pred'].item() == 0:
        return None, {}
    elif ctrl_signal['status_pred'].item() == 1:
        return 'TargetTrackSkill', {'call_flag': 1}
    elif ctrl_signal['status_pred'].item() == 2:
        return 'TargetTrackSkill', {'norm_gripper_ctrl': ctrl_signal['norm_gripper_ctrl'], 'call_flag': 1}
