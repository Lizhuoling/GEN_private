...
execution_cfg = dict(
    ...
    translation_max_dist = 0.0005,
    AlignRelativeOffset = [0, 0, 0.1],
    PutRelativeOffset = [0, 0, 0.05],
...
def get_track_target_cls_func(cur_skill):
    ...
    elif (type(cur_skill) is TargetTrackSkill and cur_skill.last_skill_info_dict['call_flag'] == 2) or (type(cur_skill) is TargetTrack_ScriptPutTargetSkill) or (type(cur_skill) is OpenGripperSkill):
...
def TargetTrack_ScriptPutTargetSkill_checkupdate(self, obs_dict, ctrl_signal):
    if ctrl_signal['norm_gripper_ctrl'][0, 0] > 0.9:
        return 'ScriptPostProcessSkill', {'norm_gripper_ctrl': 1.0, 'call_flag': 3,}  # No target will be detected as call_flag is 3
    else:
        return None, {}
