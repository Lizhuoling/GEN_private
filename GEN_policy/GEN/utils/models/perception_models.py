...
class AlohaYOLO_det():
    def __init__(self, cfg):
        ...
        self.yolo = YOLO(cfg['POLICY']['YOLO_PATH'])
        self.yolo_conf = 0.4
        ...
        if self.cfg["POLICY"]["EXTERNAL_DET"] == "YOLOv10_airphonebox":
            ...
            for status_ele in status.detach().cpu().numpy():
                ...
                if self.cfg['TASK_NAME'] == 'aloha_beverage':
                    ...
                    det_cls_name = 'lid'
                elif self.cfg['TASK_NAME'] == 'aloha_pourblueberry':
                    if status_ele == 0:
                        det_cls_name = 'juicer_cup'
                    elif status_ele == 1:
                        det_cls_name = 'blue_plate'
...
class YoloInstSegment():
    def __init__(self, cfg, model_cfg):
        self.cfg = cfg
        self.model_cfg = model_cfg
        self.yolo_model = YOLO(model_cfg['observation_cfg']['yolo_path'])
        self.cls_names = self.yolo_model.model.names
        self.imgsz = 640
        self.conf = model_cfg['observation_cfg']['yolo_conf']


    def predict_an_image(self, bgr_img):
        result = self.yolo_model.predict(bgr_img, imgsz = self.imgsz, conf = self.conf, verbose = False)[0]
        return result
    
    def seg_an_image(self, bgr_img):
        result = self.predict_an_image(bgr_img)
        cls = result.boxes.cls  # Left shape: (num_box,)
        if result.masks is not None:
            mask = result.masks.data    # Left shape: (num_box, img_h, img_w)
        else:
            mask = torch.zeros((cls.shape[0], bgr_img.shape[0], bgr_img.shape[1]), dtype = torch.bool, device = cls.device)
        return cls, mask

    def __call__(self, vision_obs_dict):
        if type(vision_obs_dict['top_rgb']) == torch.Tensor:
            top_rgb = vision_obs_dict['top_rgb'][0].cpu().numpy().astype(np.uint8)
            hand_rgb = vision_obs_dict['hand_rgb'][0].cpu().numpy().astype(np.uint8)
        else:
            top_rgb = vision_obs_dict['top_rgb']
            hand_rgb = vision_obs_dict['hand_rgb']

        top_bgr = cv2.cvtColor(top_rgb, cv2.COLOR_RGB2BGR)
        hand_bgr = cv2.cvtColor(hand_rgb, cv2.COLOR_RGB2BGR)

        top_seg_cls, top_seg_mask = self.seg_an_image(top_bgr)
        top_cls_names = [self.cls_names[cls_id] for cls_id in top_seg_cls.int().cpu().numpy()]
        top_seg_results = [(seg_cls_name, seg_mask) for seg_cls_name, seg_mask in zip(top_cls_names, top_seg_mask)]
        
        hand_seg_cls, hand_seg_mask = self.seg_an_image(hand_bgr)
        hand_cls_names = [self.cls_names[cls_id] for cls_id in hand_seg_cls.int().cpu().numpy()]
        hand_seg_results = [(seg_cls, seg_mask) for seg_cls, seg_mask in zip(hand_cls_names, hand_seg_mask)]
        return dict(top_inst_seg = top_seg_results, hand_inst_seg = hand_seg_results)
